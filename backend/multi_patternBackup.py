...
# ===================================
# Suricata Classification to MITRE ATT&CK Mapping
# ===================================

# Mapping Suricata classifications to MITRE ATT&CK Tactics and Techniques
SURICATA_TO_MITRE = {
    # Initial Access & Delivery
    "Misc activity": {"tactic": "TA0001", "technique": "T1190", "severity": "low"},
    "Not Suspicious Traffic": {"tactic": None, "technique": None, "severity": "info"},
    "Potentially Bad Traffic": {"tactic": "TA0001", "technique": "T1190", "severity": "low"},
    
    # Reconnaissance
    "Detection of a Network Scan": {"tactic": "TA0043", "technique": "T1046", "severity": "medium"},
    "Unknown Traffic": {"tactic": "TA0043", "technique": "T1595", "severity": "low"},
    
    # Initial Access / Phishing
    "Possible Social Engineering Attempted": {"tactic": "TA0001", "technique": "T1566", "severity": "medium"},
    
    # Execution
    "Possibly Unwanted Program Detected": {"tactic": "TA0002", "technique": "T1204", "severity": "medium"},
    "Executable code was detected": {"tactic": "TA0002", "technique": "T1059", "severity": "high"},
    "Web Application Attack": {"tactic": "TA0002", "technique": "T1190", "severity": "high"},
    "access to a potentially vulnerable web application": {"tactic": "TA0002", "technique": "T1190", "severity": "medium"},
    
    # Persistence
    "A Network Trojan was detected": {"tactic": "TA0003", "technique": "T1547", "severity": "critical"},
    
    # Privilege Escalation
    "Attempted User Privilege Gain": {"tactic": "TA0004", "technique": "T1068", "severity": "high"},
    "Attempted Administrator Privilege Gain": {"tactic": "TA0004", "technique": "T1068", "severity": "critical"},
    "Successful Administrator Privilege Gain": {"tactic": "TA0004", "technique": "T1068", "severity": "critical"},
    "Exploit Kit Activity Detected": {"tactic": "TA0004", "technique": "T1203", "severity": "critical"},
    
    # Defense Evasion
    "Device Retrieving External IP Address Detected": {"tactic": "TA0005", "technique": "T1082", "severity": "low"},
    "Potential Corporate Privacy Violation": {"tactic": "TA0005", "technique": "T1048", "severity": "medium"},
    
    # Credential Access
    "Crypto Currency Mining Activity Detected": {"tactic": "TA0006", "technique": "T1496", "severity": "high"},
    
    # Discovery
    "A client was using an unusual port": {"tactic": "TA0007", "technique": "T1046", "severity": "low"},
    
    # Collection
    "Attempted Information Leak": {"tactic": "TA0009", "technique": "T1530", "severity": "high"},
    "Information Leak": {"tactic": "TA0009", "technique": "T1530", "severity": "high"},
    "Large Scale Information Leak": {"tactic": "TA0009", "technique": "T1530", "severity": "critical"},
    
    # Command and Control
    "Domain Observed Used for C2 Detected": {"tactic": "TA0011", "technique": "T1071", "severity": "critical"},
    "Malware Command and Control Activity Detected": {"tactic": "TA0011", "technique": "T1071", "severity": "critical"},
    
    # Impact
    "Attempted Denial of Service": {"tactic": "TA0040", "technique": "T1498", "severity": "high"},
    "Detection of a Denial of Service Attack": {"tactic": "TA0040", "technique": "T1498", "severity": "critical"},
    
    # Generic Attack
    "Misc Attack": {"tactic": "TA0002", "technique": "T1190", "severity": "medium"},
}
...
INDEX_FIELD_MAPPINGS = {
    "palo-xsiam": {
        ...
        "alert_categories_field": "palo-xsiam.alert_categories.keyword",  # ✅ New field
        "timestamp_field": "@timestamp",
        "tactic_parser": lambda val: val.split(" - ")[0] if " - " in val else val,
        "technique_parser": lambda val: val.split(" - ")[0] if " - " in val else val,
    },
    "crowdstrike": {
        ...
        "objective_field": "crowdstrike.event.Objective.keyword",  # ✅ New field
        "event_name_field": "crowdstrike.event.Name.keyword",  # ✅ New field
        "timestamp_field": "@timestamp",
        "tactic_parser": lambda val: val,
        "technique_parser": lambda val: val,
    },
    "suricata": {
        "classification_field": "suricata.classification.keyword",  # Primary field for Suricata
        "category_field": "suricata.classification.keyword",
        "timestamp_field": "@timestamp",
        "severity_field": "suricata.alert.severity",  # Suricata has its own severity (1-3)
        "signature_field": "suricata.alert.signature.keyword",
        "use_mapping": True,  # Flag to indicate this index needs mapping
        "mapping_table": SURICATA_TO_MITRE,
    }
}
...
def get_suricata_mitre_mapping(classification: str) -> dict:
    """
    Get MITRE tactic and technique from Suricata classification.
    
    Args:
        classification: Suricata classification string
        
    Returns:
        dict with tactic, technique, and severity
    """
    mapping = SURICATA_TO_MITRE.get(classification, {
        "tactic": None,
        "technique": None, 
        "severity": "unknown"
    })
...
# Updated build_technique_query for Suricata support
def build_technique_query(field_mapping: Dict, technique_ids: List[str], start_date: str, end_date: str) -> Dict:
    ...
    # Check if this is Suricata (uses mapping table)
    if field_mapping.get("use_mapping"):
        # For Suricata: find all classifications that map to the requested techniques
        matching_classifications = []
        mapping_table = field_mapping.get("mapping_table", {})
        
        for classification, mapping in mapping_table.items():
            if mapping.get("technique") in technique_ids:
                matching_classifications.append(classification)
        
        if not matching_classifications:
        ...
        return {
            "query": {
                "bool": {
                    "must": [
                        {"terms": {field_mapping["classification_field"]: matching_classifications}},
                        {"range": {
                            field_mapping["timestamp_field"]: {
                                "gte": start_date,
                                "lte": end_date
    ...
    elif "technique_id_field" in field_mapping:
...
TACTIC_WEIGHTS = {
    ...
    "collection": 0.45,
    "discovery": 0.4,
    "resource-development": 0.3
}

def compute_severity(count: int, last_seen: str, tactic: str):
    # 1) Normalize count score
    if count >= 200:
        freq_score = 1.0
    elif count >= 100:
        freq_score = 0.8
    ...
    # 2) Recency score based on how recently the technique occurred
    rec_score = 0.0
    if last_seen:
        try:
            last_dt = datetime.fromisoformat(last_seen.replace("Z", "+00:00"))
            diff_days = (datetime.now(timezone.utc) - last_dt).days
    ...
    # 3) Tactic importance
    weight = TACTIC_WEIGHTS.get(tactic.lower(), 0.5)

    # Final score (weighted fusion)
    final_score = (freq_score * 0.5) + (rec_score * 0.3) + (weight * 0.2)
...
async def get_multi_index_technique_stats(request: MultiIndexTechniqueRequest):
    ...
    try:
        ...
        for tech in request.techniques:
            ...
            severity = compute_severity(count, last_seen, tech.tactic)
