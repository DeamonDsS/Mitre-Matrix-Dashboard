...
// --- CHANGE 1: Use only the new postgresKillChainService ---
import {
    ...
    type PhaseCoverage,
    getCoverageColorScheme, // Keep if used, otherwise remove
...
// --- CHANGE 2: Define a default empty state for the kill chain data ---
...
const CoveragedashboardStat: React.FC<KillChainDashboardProps> = ({
    ...
    // --- CHANGE 3: Initialize state with the empty structure ---
    ...
    const loadKillChainData = async () => {
        // --- CHANGE 4: Handle empty selection more gracefully ---
        ...
        try {
            ...
            // --- CHANGE 5: Set data or fallback to initial state ---
            setKillChainData(data || initialKillChainState);
            console.log("Is it have data", data);
            setMlData(null);

        } catch (err: any) {
            console.error("Error loading kill chain data:", err);
            setError(`Failed to load data: ${err.message}`);
            setKillChainData(initialKillChainState); // Fallback to empty state on error
        } finally {
            setLoading(false);
        }
    };
    ...
    // --- CHANGE 6: Use the new service's analysis functions ---
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-gray-900/90 to-gray-800/90 backdrop-blur rounded-2xl shadow-2xl border border-gray-700/50 p-8 overflow-x-auto">
                ...
                <div className="relative" style={{ minHeight: useMLPrediction && mlData ? '800px' : '700px' }}>
                    ...
                    <div className="relative flex justify-center gap-8" style={{ minWidth: 'max-content', width: '100%' }}>
                        {killChainData.phases.map((phase, index) => {
                            // --- CHANGE 7: Use the new service's metadata and threat level functions ---
                            const coverageColorScheme = getCoverageColorScheme(phase.coverage_percentage);
            ...
            {/* Footer Info */}
            {/* --- CHANGE 8: Correct the footer to use available data --- */}
