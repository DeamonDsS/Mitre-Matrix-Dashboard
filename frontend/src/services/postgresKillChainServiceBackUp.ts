...
// Service for PostgreSQL Cyber Kill Chain endpoint
...
export interface TechniqueInPhase {
  technique_id: string;
...
export interface PhaseCoverage {
  ...
  top_techniques: TechniqueInPhase[];
...
export interface KillChainSummaryPhase {
  ...
  techniques: number;
  coverage: number;
  top_technique: TechniqueInPhase | null;
  top_detection_methods: string[];
}
...
/**
 * Fetch Cyber Kill Chain coverage from PostgreSQL
 */
...
/**
 * Fetch Kill Chain summary from PostgreSQL
 */
export async function fetchPostgresKillChainSummary(
  options: {
    dayRange?: number;
    search?: string | null;
    tactic?: string;
...
): Promise<PostgresKillChainSummaryResponse> {
  try {
    const requestBody: PostgresKillChainRequest = {
      dayRange: options.dayRange || 7,
      search: options.search || null,
    ...
    const response = await fetch(`${BACKEND_API_URL}/api/postgres/kill-chain-summary`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const data: PostgresKillChainSummaryResponse = await response.json();
    return data;
  } catch (error) {
    console.error('Error fetching PostgreSQL kill chain summary:', error);
    return {
      summary: [],
      total_detections: 0,
      active_phases: 0,
...
/**
 * Sort phases by coverage percentage
 */
 ...
 */
export function getCoverageColorScheme(coveragePercent: number): {
  color: string;
  bgColor: string;
  label: string;
...
export function generateExecutiveReport(killChain: PostgresCyberKillChainResponse): {
  ...
  const executiveSummary = `Analysis of ${killChain.total_detections.toLocaleString()} detections across ${stats.activePhases} active phases revealed ${stats.uniqueTechniques} unique techniques. ${attack.isActive ? `Critical: Active attack chain detected spanning ${attack.longestChain} consecutive phases (${attack.startPhase} → ${attack.endPhase}).` : 'No continuous attack chain detected.'} Risk score: ${attack.riskScore}/100.`;
  ...
  keyFindings.push(`Average coverage: ${stats.averageCoverage}%`);
  ...
  if (stats.averageCoverage < 50) {
    recommendations.push('🎯 Improve overall detection coverage across all phases');
  }
