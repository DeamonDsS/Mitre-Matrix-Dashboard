...
const MitreAttackNavigator: React.FC = () => {
  ...
  const availableIndices = [
    { value: ".ds-winlogbeat-9.1.5-*", label: "Windows Events" },
    { value: "palo-xsiam-*", label: "Palo Alto XSIAM" },
    { value: "crowdstrike-*", label: "CrowdStrike" },
  ];

  const esIndex = selectedIndex;

  const isWindowsIndex = (index: string): boolean => {
    const pattern = detectIndexPattern(index);
  ...
  // Group techniques by parent
  const groupTechniques = (allTechniques: MitreTechniqueFramework[]) => {
    const parentTechniques = allTechniques.filter(t => isParentTechnique(t.id));
    const subTechniques = allTechniques.filter(t => !isParentTechnique(t.id));

    // Create a map of parent -> sub-techniques
    const techniqueMap = new Map<string, {
      parent: MitreTechniqueFramework;
      children: MitreTechniqueFramework[];
    }>();

    parentTechniques.forEach(parent => {
      const children = subTechniques.filter(sub => 
        getParentTechniqueId(sub.id) === parent.id
      );
      techniqueMap.set(parent.id, { parent, children });
    });
  ...
  const loadStats = async (techniquesData: MitreTechniqueFramework[]) => {
    try {
      let stats: Record<string, TechniqueStatsFramework>;

      if (isWindowsIndex(esIndex)) {
        console.log("🪟 Using Legacy Endpoint for Windows Events");
        stats = await fetchAllTechniqueStatsWithDateRange(
          techniquesData,
          esIndex,
          dateRange
        );
      } else {
        console.log("🚀 Using Multi-Index Endpoint for", detectIndexPattern(esIndex));
        stats = await fetchMultiIndexTechniqueStats(
          techniquesData,
          esIndex,
          dateRange
        );
      ...
      try {
        const fallbackStats = await fetchAllTechniqueStatsWithDateRange(
          techniquesData,
          esIndex,
          dateRange
        );
        setTechniqueStats(fallbackStats);
      } catch (fallbackError) {
  ...
  const loadOverallStats = async () => {
    try {
      ...
      if (isWindowsIndex(esIndex)) {
        console.log("🪟 Fetching Windows stats from legacy endpoint");
        ...
        console.log("🚀 Fetching stats from multi-index endpoint");
        const statsData = await fetchMultiIndexStats(esIndex, {
          search: undefined,
          tactic: "all",
          severity: "all",
          dayRange: dayRange,
        ...
        const legacyStats = convertToLegacyStats(statsData);
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <select
                ...
                className="px-3 py-1.5 bg-gray-700 text-white rounded hover:bg-gray-600 transition-colors text-sm focus:outline-none focus:ring-1 focus:ring-red-500"
              >
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <div className="bg-gray-900 rounded p-3">
              ...
              <div className="text-sm font-bold text-white truncate">
                {availableIndices.find((idx) => idx.value === selectedIndex)
                  ?.label || selectedIndex}
              </div>
            </div>
            <div className="bg-gray-900 rounded p-3">
